...
- 内容校对批次(2026-09-04,经用户核对原始数据):Hero KPI「意图识别准确率」→「结果采纳率」;阶段一幻觉率→上下文读取成功率(68.20%→93.70%);阶段二换任务解析准确率/Loop 完成率/单步工具成功率;阶段三换 Skill 保存率(18.60%)/复用率(41.30%,含分母口径);阶段四换 Agent 任务完成率(强调副本)+用户中断率(31.50%→9.60%);幻觉率/留存类旧指标常量全站删除
