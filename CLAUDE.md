...
- 样式细节修改完成(2026-09-04):Hero 文案与 0→1 KPI 展示、医学生章节校徽时间线节点(复旦圆徽/华科椭圆徽)、985·211 teal 强调标签、跨界章节 isDelta 变化量语义(pp 单位、数值列 —、变化列 ±)、「战役」→「阶段」全站替换、荣誉章节奖项卡片荣誉金图标(CSS 蒙版,跟随深浅色 token)
- 内容校对批次(2026-09-04,经用户核对原始数据):Hero KPI「意图识别准确率」→「结果采纳率」;阶段一为意图识别准确率+上下文读取成功率(68.20%→93.70%);阶段二换任务解析准确率/Loop 完成率/单步工具成功率;阶段三换 Skill 保存率(18.60%)/复用率(41.30%,含分母口径);阶段四换 Agent 任务完成率(强调副本)+关键字段准确率(55.60%→86.80%,自阶段一移入);幻觉率/留存类旧指标常量全站删除
